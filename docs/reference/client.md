...
## Internal dialect dispatch

`Client` owns conversion, HTTP, status checks, and response parsing. It selects a private `WireDialect` implementation from the public `Dialect` enum; neither that trait nor the wire modules are part of the public contract. Streaming uses the same dialect selection to choose a private decoder.

Applications configure supported wire formats through `Dialect` and endpoints through `EndpointConfig`. See [Adding a dialect](../internals/adding-a-dialect.md) for crate-internal maintenance guidance.
