...
/// Keywords OpenAI's strict mode rejects that only *constrain* a value.
///
/// Dropping these loosens validation without changing the shape the model must
/// produce, so a schema still describes the same type after the round trip.
...
/// generated by a crate that writes to spec, such as `schemars` — is *rejected*
...
/// ```
///
/// Freyja does not generate schemas, so this takes one you already have from
/// anywhere and makes it acceptable. It is idempotent: a schema already in the
/// subset passes through unchanged.
///
/// ```
/// use freyja::{strict_schema, GenerateRequest, Message, ResponseFormat, Role};
/// use serde_json::json;
///
...
/// }));
///
/// // `note` was optional. Strict mode has no optional, so it is now nullable
/// // and required, which means the same thing to a caller deserializing it.
/// assert_eq!(schema["additionalProperties"], false);
/// assert_eq!(schema["required"], json!(["name", "note"]));
/// assert_eq!(schema["properties"]["note"]["type"], json!(["string", "null"]));
///
/// let request = GenerateRequest::new()
///     .message(Message::text(Role::User, "Describe a crate."))
///     .response_format(ResponseFormat::JsonSchema {
///         name: "crate_note".into(),
///         schema,
///         strict: true,
///     });
/// ```
...
/// # What it changes
///
/// - `additionalProperties: false` on every object, which strict mode requires
///   and rejects the absence of, nested objects included.
/// - Every property moved into `required`. A property that was optional first
///   has `null` added to its type, so "may be absent" becomes "may be null"
///   rather than "must be present".
/// - `oneOf` becomes `anyOf`. Strict mode permits only the latter, and for the
///   tagged unions a derive emits — where the variants are mutually exclusive —
///   the two mean the same thing.
/// - `uniqueItems`, `minProperties`, `maxProperties`, `dependentRequired`, and
...
/// rejected by strict mode, and are **not** removed. Unlike the list above,
/// dropping them would change which documents the schema describes, and
/// silently sending a different contract than the caller wrote is worse than
/// the endpoint saying no. Those arrive as [`ProviderError::BadRequest`] naming
/// the keyword.
///
/// # Scope
///
/// The rules are OpenAI's. Gemini accepts schemas with or without them, and
/// Anthropic is unverified, so one function serves all three today. If a vendor
/// ever wants something different this gains a dialect parameter.
...
/// Moves every property into `required`, making the newly-required ones
/// nullable so the schema still describes what it did before.
fn require_every_property(map: &mut serde_json::Map<String, Value>) {
    ...
    // Only now, once `required` no longer borrows the map.
...
/// Adds `null` to a property's type, where it has a plain one to add it to.
///
/// A property described by `$ref` or a union has no `type` to extend, and is
/// left as it is: a derive already spells an optional reference as a union
/// including `null`, and a hand-written one is the caller's to say.
...
mod tests {
    ...
    fn strict_schema_supplies_what_the_endpoint_demands() {
        // Verified against OpenAI: omitting `additionalProperties`, or setting
        // it true, or leaving a property out of `required`, each rejects the
        // whole request.
    ...
    #[test]
    fn an_optional_property_becomes_nullable_rather_than_mandatory() {
        // Strict mode has no optional. Forcing `note` into `required` alone
        // would demand a value the caller said was not needed; adding null to
        // its type keeps the original meaning.
        let schema = strict_schema(serde_json::json!({
            "type": "object",
            "properties": {"name": {"type": "string"}, "note": {"type": "string"}},
            "required": ["name"]
    ...
    #[test]
    fn already_nullable_properties_are_not_doubled() {
        let schema = strict_schema(serde_json::json!({
            "type": "object",
    ...
    #[test]
    fn nested_objects_and_defs_are_reached() {
        // The rejection names its context -- `In context=('properties', 'n')`
        // -- so a top-level-only pass would fail on the first nested object.
        let schema = strict_schema(serde_json::json!({
            "type": "object",
        ...
        assert_eq!(schema["$defs"]["Author"]["additionalProperties"], false);
    }

    #[test]
    fn one_of_becomes_any_of() {
        // Strict mode permits `anyOf` and refuses `oneOf`. For the mutually
        // exclusive variants a derive emits, they describe the same set.
        let schema = strict_schema(serde_json::json!({
            "type": "object",
            "properties": {"either": {"oneOf": [{"type": "string"}, {"type": "integer"}]}},
            "required": ["either"]
        }));

        let either = &schema["properties"]["either"];
        assert!(either.get("oneOf").is_none());
        assert_eq!(
            either["anyOf"],
    ...
    #[test]
    fn constraint_only_keywords_go_and_the_rest_stay() {
        // Probed one at a time against OpenAI: everything in the second list
        // is accepted, so removing any of it would discard guidance the model
        // uses for nothing.
        let schema = strict_schema(serde_json::json!({
            "type": "object",
            "properties": {
                "tags": {
                    ...
                    "description": "keep me", "format": "custom",
                    "pattern": "^a", "enum": ["a"], "minimum": 0, "default": "a"
                }
            },
            "required": ["tags"],
            "minProperties": 1
        ...
        for kept in [
            ...
            assert!(tags.get(kept).is_some(), "{kept} is accepted and must stay");
        }
    }

    #[test]
    fn structural_keywords_are_left_for_the_endpoint_to_refuse() {
        // These are rejected too, but dropping them would change which
        // documents the schema describes. Sending a different contract than
        // the caller wrote is worse than a clear refusal.
        let schema = strict_schema(serde_json::json!({
            "type": "object",
            "properties": {
                "a": {"allOf": [{"type": "string"}]},
    ...
    #[test]
    fn strict_schema_is_idempotent() {
        ...
        let once = strict_schema(original);
        let twice = strict_schema(once.clone());

        assert_eq!(once, twice, "a schema already in the subset must not move");
    }

    #[test]
    fn a_schema_without_properties_is_left_alone() {
        // `required: []` on a property-less object is valid, and inventing a
        // `required` for one would be wrong.
        let schema = strict_schema(serde_json::json!({"type": "string"}));
